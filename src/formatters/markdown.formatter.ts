...
export function formatAsMarkdown(reports: DevMetricsReport[]): string {
  ...
  for (const report of reports) {
    ...
    output.push(`| 📝 Last Commit | ${report.github.lastCommitDate ? new Date(report.github.lastCommitDate).toLocaleDateString() : 'N/A'} |`);
    ...
    output.push(`| ⏰ Last Transaction | ${report.rootstock.lastTransactionTimestamp ? new Date(report.rootstock.lastTransactionTimestamp).toLocaleDateString() : 'N/A'} |`);
